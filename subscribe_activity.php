...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($child) {
        $stmt = $pdo->prepare("INSERT INTO children_activities (child_id, activity_id) VALUES (?, ?)");
        if ($stmt->execute([$child_id, $activity_id])) {
            $_SESSION['message'] = "Successfully subscribed to the activity.";
            $_SESSION['message_type'] = "success";
        } else {
            $_SESSION['message'] = "Failed to subscribe to the activity.";
            $_SESSION['message_type'] = "danger";
        }
    } else {
        $_SESSION['message'] = "Invalid child or parent.";
        $_SESSION['message_type'] = "danger";
    }
